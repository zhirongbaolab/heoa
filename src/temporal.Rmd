...
# divide systemically changing genes into 3 patterns: glo down, glo down with low expression at heart/endothelium/blood, glo up
...
lex_ratio<- .6
scg_pat<- list( scg[rowSums(clu_temp_fd[scg,])<0 & rowSums(clu_temp_fd[scg,l4s]==0)>=lex_ratio*length(l4s) ],
scg[rowSums(clu_temp_fd[scg,])<0 & rowSums(clu_temp_fd[scg,l4s]==0)<lex_ratio*length(l4s) ],
scg[rowSums(clu_temp_fd[scg,])>0] )
sapply( scg_pat<- lapply(scg_pat, function(x){ scg_od[scg_od %in% x]}), length) # order:  80 388  78
...
scg_out<- cbind( 'stage 3', rep(c('down','down','up'), sapply(scg_pat,length)), unlist(scg_pat), ge2an[unlist(scg_pat)], rep(c(1:3), sapply(scg_pat,length)), clu_temp_fd[unlist(scg_pat),] )
