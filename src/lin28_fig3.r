#' LIN28A Figure 3: systemically changing genes and developmental processes in embryonic stages.

#' For the definition of systemicall changing genes: see lin28.r
# Fig. 3a: Number of systemically changing genes
...
# Fig. 3b: expression pattern of systemically changing gens in human
...
# Fig. 3c: pathways that are positively correlated to the expression of LIN28A mRNA
...
names(gind)<-c('up','dp')
go_cate_od<- 1:length(unique(go_cate))
names(go_cate_od)<-c('cell cycle','gene expression','translation and post-translation','metabolism','signaling','cellular component','mitochondrion','homeostasis','other')
...
for(i in c(1,8,14)){ # for cell cycle, mRNA splicing, translation
